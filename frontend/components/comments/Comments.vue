...
<template>
  <div :class="`comment ${comment.replies.length ? 'comment--haveReplies' : ''}`">
    ...
    <div v-if="comment.replies.length" class="comment__replies-wrapp">
      ...
      <div class="comment__replies-more" v-if="hideReplies" @click="hideReplies = !hideReplies">
        {{
          comment.replies.length === 1
            ? t('showRepliesOne', { count: comment.replies.length })
            : t('showRepliesAll', { count: comment.replies.length })
        }}
      </div>
      <div
    ...
    <div v-if="!hideReplies && authStore.isAuthenticated" class="comment__input">
